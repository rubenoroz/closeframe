...
export async function generateQRCode(url: string): Promise<Buffer> {
    ...
    const qrData = QRCode.create(url, {
        ...
        // version: 5  <-- REMOVED to allow auto-scaling (e.g. to version 6 or higher)
    });
    ...
    // Inside catch block for fallback:
    ...
    try {
        ...
        return await QRCode.toBuffer(url, {
            errorCorrectionLevel: 'H',
            margin: 2,
            width: QR_SIZE // Keep width to match expected output size, lib handles version
        });
    } catch (finalError) {
